...
if (isset($_POST['signin']) || isset($_SESSION['confirm'])) {
    ...
    $numrow9 = mysqli_num_rows($osas); 

    $guidance = mysqli_query($db, "SELECT * FROM tbl_guidances WHERE username = '$username'");
    $numrow10 = mysqli_num_rows($guidance); 
    ...
    if ($numrow > 0) {
        ...
        while ($row = mysqli_fetch_array($accounting)) {
            ...
            if ($hashedPwdCheck == false) {
                ...
                // dagdag eto session name for acctg
        ...
        while ($row = mysqli_fetch_array($osas)) {
            $hashedPwdCheck = password_verify($password, $row['password']);
            if ($hashedPwdCheck == false) {
                $_SESSION['sessionP'] = true;
                header("location: ../sign-in.php");
                ...
                $_SESSION['role'] = "OSA";
                $_SESSION['userid'] = $row['osas_id'];
                $_SESSION['name'] = $row['osas_lastname'] . ", " . $row['osas_firstname'];
            }
            header("location: ../../dashboard/index.php");
        }

    } elseif ($numrow10 > 0) {
        while ($row = mysqli_fetch_array($guidance)) {
            $hashedPwdCheck = password_verify($password, $row['password']);
            if ($hashedPwdCheck == false) {
                $_SESSION['sessionP'] = true;
                header("location: ../sign-in.php");
                ...
                $_SESSION['userid'] = $row['guidance_id'];
                $_SESSION['name'] = $row['guidance_lastname'] . ", " . $row['guidance_firstname'];
            }
            header("location: ../../dashboard/index.php");
        }    
