...
if (isset($_POST['saveImg'])) {
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        if ($_SESSION['role'] == "Super Administrator" || $_SESSION['role'] == "Guidance") {
...
if (isset($_POST['save'])) {
    ...
    $guidance = mysqli_real_escape_string($db, $_POST['guidance_no']);
    ...
    $getAllUsername = mysqli_query($db, "SELECT username FROM tbl_admissions WHERE username = '$username' UNION ALL SELECT username FROM tbl_deans WHERE username = '$username' UNION ALL SELECT username FROM tbl_faculties WHERE username = '$username' UNION ALL SELECT username FROM tbl_admins WHERE username = '$username' UNION ALL SELECT username FROM tbl_students WHERE username = '$username' UNION ALL SELECT username FROM tbl_presidents WHERE username = '$username' UNION ALL SELECT username FROM tbl_accounting WHERE username = '$username' UNION ALL SELECT username FROM tbl_super_admins WHERE username = '$username' UNION ALL SELECT username FROM tbl_guidances WHERE username = '$username'") or die(mysqli_error($db));
    ...
    if ($check == 0) {
        ...
        if ($getID == $guidance_id || $check2 < 1) {
            $updateInfo = mysqli_query($db, " UPDATE tbl_guidances SET guidance_lastname='$lastname',guidance_firstname='$firstname', guidance_middlename='$middlename', guidance_no = '$guidance_no', position = '$position', status = '$status', email='$email', username='$username', updated_by = '$updated_by', last_updated = CURRENT_TIMESTAMP WHERE guidance_id = '$guidance_id'") or die(mysqli_error($db));
