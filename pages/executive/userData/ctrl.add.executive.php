            ...
            if (!empty($_POST['faculty_no']) && !empty($_POST['position']) && !empty($_POST['status'])) {
                ...
                if (!empty($_POST['username']) && !empty($_POST['password'])) {
                    ...
                    if ($password == $confirmPass) {
                        ...
                        $insertExecutive          = mysqli_query($db, "INSERT INTO tbl_faculties_staff (img, faculty_firstname, faculty_middlename, faculty_lastname, faculty_no, position, status, activation_code, email, username, password, created_at) VALUES ('$image', '$fname', '$mname', '$lname', '$faculty_no', '$position', '$status', '', '$email', '$username', '$hashedPwd', CURRENT_TIMESTAMP)") or die(mysqli_error($db));
                        $_SESSION['successAdd'] = true;
                        header("location: ../add.executive.php");
                    } else {
